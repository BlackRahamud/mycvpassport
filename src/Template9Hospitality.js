...
export function PreviewMinimalistPurple({ cv, mobileMode = false }) {
  ...
  const textColor = isPlaceholder ? BORDER_LIGHT : MAIN_TEXT;
  const accentColor = isPlaceholder ? BORDER_LIGHT : DEEP_PURPLE;
  ...
  return (
    ...
    >
      {/* Header: Clean white, Name bold left, title inline italic */}
      <header style={{ marginBottom: "6mm" }}>
        <div style={{ display: "flex", alignItems: "baseline", gap: "12px", marginBottom: "6px" }}>
          <h1 style={{ fontSize: pt(24), fontWeight: "bold", margin: 0, color: textColor }}>{cv.name || "YOUR NAME"}</h1>
          <span style={{ fontSize: pt(14), fontStyle: "italic", color: greyColor }}>{cv.title || "Professional Title"}</span>
        </div>

        {/* Contact row below with small separators */}
        <div style={{ display: "flex", flexWrap: "wrap", columnGap: "12px", fontSize: pt(9.5), color: greyColor }}>
            ...
            <div key={i} style={{ marginBottom: "6mm", pageBreakInside: "avoid" }}>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline" }}>
                <div style={{ fontSize: pt(11) }}>
                  ...
                  <span style={{ fontStyle: "italic", color: greyColor }}> — {exp.company || "Company"}</span>
                </div>
      ...
      {(skills.length > 0 || isPlaceholder) && (
        <section data-section="competencies" style={{ pageBreakInside: "avoid" }}>
          ...
          <div style={{ fontSize: pt(10), color: textColor, lineHeight: 1.8 }}>
            {(isPlaceholder ? ["Skill One", "Skill Two", "Skill Three"] : skills).map((s, i, arr) => (
              <span key={i}>
                {s}
                {i < arr.length - 1 ? "  •  " : ""}
              </span>
