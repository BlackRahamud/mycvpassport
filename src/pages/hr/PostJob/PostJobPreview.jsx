...
function PopulatedJobCard({ job }) {
  ...
  const list = { initial: {}, animate: { transition: { staggerChildren: 0.08, delayChildren: 0.18 } } };
  const item = {
    ...
    <div className="pj-preview">
      ...
      >
        <AnimatePresence mode="wait">
          ...
          {isPopulated && (
            <motion.div key="populated" initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -6, transition: { duration: 0.18 } }} transition={{ duration: 0.36, ease: [0.4, 0, 0.2, 1] }}>
              <PopulatedJobCard job={job || {}} />
            </motion.div>
          )}
        </AnimatePresence>
      </motion.div>
